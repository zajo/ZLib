...
#include "extern/zlib-1.2.11/zlib.h"
...
public:
    ...
    public:
        ZLib(Split in, Split check, Flush flush = Flush::sync)
            : in_(in)
            , check_(check)
            , flush_(flush)
        {
    ...
    testInflate(IDecompressor&)
    {
        ...
        }
        {
            ...
            m.window(9);
            m.strategy(Z_DEFAULT_STRATEGY);
            m(ZLib{full, once, Flush::block}, check);
        }
        {
...
#if 0
        {
            ...
            m.window(9);
            m.strategy(Z_DEFAULT_STRATEGY);
            m(ZLib{full, once, Flush::trees}, check);
        }
#endif
...
TEST_SUITE(inflate_stream_test, "zlib.inflate_stream");
